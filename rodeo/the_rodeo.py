...
class the_rodeo:
    ...
    def get_TS(self, point, h):
        ...
        for bi in range(d):
            ...
            Zd = self._stat_model.evaluate_bandwidth_derivative(
                point, self._data, h, bi
            ).flatten()
    ...
    def local_rodeo(self, point, beta, h0=None):
        ...
        if need_distances:
            ...
            for j in range(len(self._data)):
                ...
                val_sum = accumulate(
                    (
                        abs(sorted_vals[vi] - point[j]) * sorted_weights[vi]
                        for vi in range(n)
                    )
                )
                ...
                s_sum = accumulate(
                    (
                        sorted_weights[vi]
                        * (abs(sorted_vals[vi] - point[j]) - avg_diff) ** 2
                        for vi in range(n)
                    )
                )
        ...
        while len(A) > 0:
            ...
            for bi in A:
                ...
                s = np.sqrt(
                    accumulate(
                        (
                            self._weights[vi] * ((Zs[vi] - mu_star) ** 2.0)
                            for vi in range(n)
                        )
                    )
                    * v2
